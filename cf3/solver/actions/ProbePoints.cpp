...
#include "solver/actions/ProbePoints.hpp"
...
common::ComponentBuilder < ProbePoints, common::Action, solver::actions::LibActions > ProbePoints_Builder;

////////////////////////////////////////////////////////////////////////////////////////////

ProbePoints::ProbePoints( const std::string& name  ) : common::Action(name)
{
  ...
  options().add("x_coordinate",std::vector<Real>())
    .pretty_name("xcoordinate")
    .description("x-coordinate to interpolate fields to")
    .mark_basic();

  options().add("y_coordinate",std::vector<Real>())
    .pretty_name("ycoordinate")
    .description("y-coordinate to interpolate fields to")
    .mark_basic();

  options().add("z_coordinate",std::vector<Real>())
    .pretty_name("zcoordinate")
    .description("z-coordinate to interpolate fields to")
    .mark_basic();
    
  options().add("dict",m_dict)
      .description("Dictionary that will be probed")
      .link_to(&m_dict)
      .attach_trigger( boost::bind( &ProbePoints::configure_point_interpolator, this ) );
  ...
  regist_signal ( "create_post_processor" )
      ...
      .connect   ( boost::bind ( &ProbePoints::signal_create_post_processor,    this, _1 ) )
      .signature ( boost::bind ( &ProbePoints::signature_create_post_processor, this, _1 ) );
...
void ProbePoints::configure_point_interpolator()
{
...
void ProbePoints::execute()
{
  ...
  RealVector coord(opt_coord.size()); //get the size
...
//  std::cout << PE::Comm::instance().rank() << ":  glb_idx = " << glb_idx << std::endl;
  ...
  // Do all post-processing actions, which could add more properties to the ProbePoints,
...
void ProbePoints::set(const std::string& var_name, const Real& var_value)
{
...
void ProbePoints::set(const std::string& var_name, const std::vector<Real>& var_values)
{
...
Handle<ProbePointsPostProcessor> ProbePoints::create_post_processor(const std::string& name, const std::string& builder)
{
  Handle<ProbePointsPostProcessor> p = create_component(name,builder)->handle<ProbePointsPostProcessor>();
  p->options().set("ProbePoints",this->handle());
  return p;
}

////////////////////////////////////////////////////////////////////////////////

void ProbePoints::signal_create_post_processor(SignalArgs &args)
{
  SignalOptions signal_options(args);

  Handle<ProbePointsPostProcessor> pp =
...
void ProbePoints::signature_create_post_processor(SignalArgs &args)
{
  SignalOptions options(args);

  options.add("name", std::string("function"));
  options.add("type", std::string("cf3.mesh.ProbePointsFunction"));
}
...
ProbePointsPostProcessor::ProbePointsPostProcessor(const std::string &name) : common::Action(name)
{
  options().add("ProbePoints",m_ProbePoints).link_to(&m_ProbePoints);
}

////////////////////////////////////////////////////////////////////////////////

void ProbePointsPostProcessor::set(const std::string& var_name, const Real& var_value)
{
  m_ProbePoints->set(var_name,var_value);
}

////////////////////////////////////////////////////////////////////////////////

void ProbePointsPostProcessor::set(const std::string& var_name, const std::vector<Real>& var_values)
{
  m_ProbePoints->set(var_name,var_values);
}
