...
generator = target_domain.create_component('Generator', 'cf3.mesh.GeneratePlane3D')
generator.mesh = target_mesh.uri()
generator.origin = [0., h, 0.]
generator.normal = [0., 1., 0.]
generator.size = [x_size, z_size]
generator.segments = [x_segs, z_segs]
generator.execute()
...
for i in range(len(tgt_coords)):
  if tgt_ranks[i] == my_rank:
    print tgt_coords[i][0], tgt_coords[i][1], tgt_coords[i][2], tgt_test[i][0]
