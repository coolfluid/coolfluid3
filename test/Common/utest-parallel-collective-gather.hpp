...
{
  PEProcessSortedExecute(mpi::PE::instance(),-1,CFinfo << "Testing gather " << irank << "/" << nproc << CFendl; );
}
...
BOOST_AUTO_TEST_CASE( gather_ptr_constant )
{
  ...
  for (r=0; r<nproc; r++){
    ...
    ptr_tmprcv=mpi::gather(mpi::PE::instance(), ptr_snddat, nproc, (double*)0, r);
    ...
    mpi::gather(mpi::PE::instance(), ptr_snddat, nproc, ptr_tmprcv, r);
    if (r==irank) for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );

    ptr_tmprcv=new double[nproc*nproc];
    if (r==irank) for (i=0; i<nproc*nproc; i++) ptr_tmprcv[i]=0.;
    for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
    mpi::gather(mpi::PE::instance(), ptr_tmprcv, nproc, ptr_tmprcv, r);
    if (r==irank) for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );

    if (r==irank) delete[] ptr_tmprcv;
    if (r==irank) ptr_tmprcv=0;
    ptr_tmprcv=(double*)mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, nproc, (char*)0, r, sizeof(double));
    if (r==irank) for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );
    if (r!=irank) BOOST_CHECK_EQUAL( ptr_tmprcv , (double*)0 );

    if (r==irank) for (i=0; i<nproc*nproc; i++) ptr_tmprcv[i]=0.;
    mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, nproc, (char*)ptr_tmprcv, r, sizeof(double));
    if (r==irank) for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );

    ptr_tmprcv=new double[nproc*nproc];
    if (r==irank) for (i=0; i<nproc*nproc; i++) ptr_tmprcv[i]=0.;
    for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
    mpi::gather(mpi::PE::instance(), (char*)ptr_tmprcv, nproc, (char*)ptr_tmprcv, r, sizeof(double));
    if (r==irank) for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );
  }
}
...
BOOST_AUTO_TEST_CASE( gather_vector_constant )
{
  ...
  for (r=0; r<nproc; r++){
    ...
    mpi::gather(mpi::PE::instance(), vec_snddat, vec_tmprcv);
    ...
    mpi::gather(mpi::PE::instance(), vec_tmprcv, vec_tmprcv);
    for (i=0; i<nproc*nproc; i++)  BOOST_CHECK_EQUAL( vec_tmprcv[i] , vec_rcvdat[i] );

    vec_tmprcvchr.resize(0);
    vec_tmprcvchr.reserve(0);
    mpi::gather(mpi::PE::instance(), vec_snddatchr, vec_tmprcvchr );
    ...
    mpi::gather(mpi::PE::instance(), vec_tmprcvchr, vec_tmprcvchr );
    for (i=0; i<nproc*nproc; i++) BOOST_CHECK_EQUAL( ((double*)(&vec_tmprcvchr[0]))[i], vec_rcvdat[i] );
}
...
/*
  ...
  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  ptr_tmprcv=mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, (double*)0, ptr_rcvcnt);
  ...
  mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, ptr_tmprcv, ptr_rcvcnt);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[k]=0.;
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  mpi::gather(mpi::PE::instance(), ptr_tmprcv, sndcnt, ptr_tmprcv, ptr_rcvcnt);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  ptr_tmprcv=mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, (double*)0, ptr_tmpcnt);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[k]=0.;
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, ptr_tmprcv, ptr_tmpcnt);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );
  ...
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), ptr_tmprcv, sndcnt, ptr_tmprcv, ptr_tmpcnt);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  ptr_tmprcv=mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, ptr_sndmap, (double*)0, ptr_rcvcnt, ptr_rcvmap);
  ...
  mpi::gather(mpi::PE::instance(), ptr_snddat, sndcnt, ptr_sndmap, ptr_tmprcv, ptr_rcvcnt, ptr_rcvmap);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[i*nproc+j] , ptr_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[i*nproc+j]=0.;
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  mpi::gather(mpi::PE::instance(), ptr_tmprcv, sndcnt, ptr_sndmap, ptr_tmprcv, ptr_rcvcnt, ptr_rcvmap);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[i*nproc+j] , ptr_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init

  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  ptr_tmprcv=(double*)mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, (char*)0, ptr_rcvcnt, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[k]=0.;
  mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, (char*)ptr_tmprcv, ptr_rcvcnt, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[k]=0.;
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  mpi::gather(mpi::PE::instance(), (char*)ptr_tmprcv, sndcnt, (char*)ptr_tmprcv, ptr_rcvcnt, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  ptr_tmprcv=(double*)mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, (char*)0, ptr_tmpcnt, sizeof(double));
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[k]=0.;
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, (char*)ptr_tmprcv, ptr_tmpcnt, sizeof(double));
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );
  ...
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  for(i=0; i<nproc; i++) ptr_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), (char*)ptr_tmprcv, sndcnt, (char*)ptr_tmprcv, ptr_tmpcnt, sizeof(double));
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmpcnt[i] , ptr_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[k] , ptr_rcvdat[k] );

  delete[] ptr_tmprcv;
  ptr_tmprcv=0;
  ptr_tmprcv=(double*)mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, ptr_sndmap, (char*)0, ptr_rcvcnt, ptr_rcvmap, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[i*nproc+j] , ptr_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init

  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[i*nproc+j]=0.;
  mpi::gather(mpi::PE::instance(), (char*)ptr_snddat, sndcnt, ptr_sndmap, (char*)ptr_tmprcv, ptr_rcvcnt, ptr_rcvmap, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[i*nproc+j] , ptr_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) ptr_tmprcv[i*nproc+j]=0.;
  for (i=0; i<nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
  mpi::gather(mpi::PE::instance(), (char*)ptr_tmprcv, sndcnt, ptr_sndmap, (char*)ptr_tmprcv, ptr_rcvcnt, ptr_rcvmap, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<ptr_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ptr_tmprcv[i*nproc+j] , ptr_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init
*/
}
...
/*
  ...
  vec_tmprcv.resize(0);
  vec_tmprcv.reserve(0);
  mpi::gather(mpi::PE::instance(), vec_snddat, sndcnt, vec_tmprcv, vec_rcvcnt);
  ...
  mpi::gather(mpi::PE::instance(), vec_tmprcv, sndcnt, vec_tmprcv, vec_rcvcnt);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( vec_tmprcv[k] , vec_rcvdat[k] );

  vec_tmprcv.resize(0);
  vec_tmprcv.reserve(0);
  for(i=0; i<nproc; i++) vec_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), vec_snddat, sndcnt, vec_tmprcv, vec_tmpcnt);
  ...
  for (i=0; i<nproc; i++) vec_tmprcv[i]=vec_snddat[i];
  for(i=0; i<nproc; i++) vec_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), vec_tmprcv, sndcnt, vec_tmprcv, vec_tmpcnt);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( vec_tmpcnt[i] , vec_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( vec_tmprcv[k] , vec_rcvdat[k] );

  vec_tmprcv.resize(0);
  vec_tmprcv.reserve(0);
  mpi::gather(mpi::PE::instance(), vec_snddat, sndcnt, vec_sndmap, vec_tmprcv, vec_rcvcnt, vec_rcvmap);
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) vec_tmprcv[i*nproc+j]=0.;
  for (i=0; i<nproc; i++) vec_tmprcv[i]=vec_snddat[i];
  mpi::gather(mpi::PE::instance(), vec_tmprcv, sndcnt, vec_sndmap, vec_tmprcv, vec_rcvcnt, vec_rcvmap);
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( vec_tmprcv[i*nproc+j] , vec_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init

  vec_tmprcvchr.resize(0);
  vec_tmprcvchr.reserve(0);
  mpi::gather(mpi::PE::instance(), vec_snddatchr, sndcnt, vec_tmprcvchr, vec_rcvcnt, sizeof(double));
  ...
  mpi::gather(mpi::PE::instance(), vec_tmprcvchr, sndcnt, vec_tmprcvchr, vec_rcvcnt, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ((double*)(&vec_tmprcvchr[0]))[k] , vec_rcvdat[k] );

  vec_tmprcvchr.resize(0);
  vec_tmprcvchr.reserve(0);
  for(i=0; i<nproc; i++) vec_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), vec_snddatchr, sndcnt, vec_tmprcvchr, vec_tmpcnt, sizeof(double));
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( vec_tmpcnt[i] , vec_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ((double*)(&vec_tmprcvchr[0]))[k] , vec_rcvdat[k] );
  ...
  for (i=0; i<nproc; i++) ((double*)(&vec_tmprcvchr[0]))[i]=vec_snddat[i];
  for(i=0; i<nproc; i++) vec_tmpcnt[i]=-1;
  mpi::gather(mpi::PE::instance(), vec_tmprcvchr, sndcnt, vec_tmprcvchr, vec_tmpcnt, sizeof(double));
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( vec_tmpcnt[i] , vec_rcvcnt[i] );
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ((double*)(&vec_tmprcvchr[0]))[k] , vec_rcvdat[k] );

  vec_tmprcvchr.resize(0);
  vec_tmprcvchr.reserve(0);
  mpi::gather(mpi::PE::instance(), vec_snddatchr, sndcnt, vec_sndmap, vec_tmprcvchr, vec_rcvcnt, vec_rcvmap, sizeof(double));
  ...
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) ((double*)(&vec_tmprcvchr[0]))[i*nproc+j]=0.;
  for (i=0; i<nproc; i++) ((double*)(&vec_tmprcvchr[0]))[i]=vec_snddat[i];
  mpi::gather(mpi::PE::instance(), vec_tmprcvchr, sndcnt, vec_sndmap, vec_tmprcvchr, vec_rcvcnt, vec_rcvmap, sizeof(double));
  for (i=0, k=0; i<nproc; i++) for (j=0; j<vec_rcvcnt[i]; j++, k++) BOOST_CHECK_EQUAL( ((double*)(&vec_tmprcvchr[0]))[i*nproc+j] , vec_rcvdat[k] ); // i*nproc+j is not a bug, check reason at init
*/
}
