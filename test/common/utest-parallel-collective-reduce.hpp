...
BOOST_AUTO_TEST_CASE( reduce_most_common_ops )
{
  ...
  iresult=-1;
  dresult=-1.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), &ival, 1, &iresult, r);
  BOOST_CHECK_EQUAL( iresult, itest );
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), &dval, 1, &dresult, r);
  ...
  iresult=-1;
  dresult=-1.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::multiplies(), &ival, 1, &iresult, r);
  BOOST_CHECK_EQUAL( iresult, itest );
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::multiplies(), &dval, 1, &dresult, r);
  BOOST_CHECK_EQUAL( dresult, dtest );
  ...
  iresult=-1;
  dresult=-1.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::max(), &ival, 1, &iresult, r);
  BOOST_CHECK_EQUAL( iresult, itest );
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::max(), &dval, 1, &dresult, r);
  BOOST_CHECK_EQUAL( dresult, dtest );
  ...
  iresult=-1;
  dresult=-1.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::min(), &ival, 1, &iresult, r);
  BOOST_CHECK_EQUAL( iresult, itest );
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::min(), &dval, 1, &dresult, r);
  BOOST_CHECK_EQUAL( dresult, dtest );
}
...
BOOST_AUTO_TEST_CASE( reduce_operator_of_class )
{
  ...
  in[1].init();
  in[2].init();
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), in, 3, out, r);
...
BOOST_AUTO_TEST_CASE( reduce_ptr_constant )
{
  ...
  for(r=0; r<nproc; r++) {
    if (r==irank) {
      ptr_tmprcv=PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, (double*)0, r);
    } else {
      double* dummy=PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, (double*)0, r);
  ...
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, ptr_tmprcv, r);
  for (i=0; i<2*nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );

  for(r=0; r<nproc; r++) {
    for (i=0; i<2*nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
    PE::Comm::instance().reduce(PE::plus(), ptr_tmprcv, sndcnt, ptr_tmprcv, r);
    if (irank==r) {
      for (i=0; i<2*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[i] , ptr_rcvdat[i] );
    } else {
  ...
  for(r=0; r<nproc; r++) {
    if (r==irank) {
      ptr_tmprcv2=PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, (double*)0, r, 2);
    } else {
      double* dummy=PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, (double*)0, r, 2);
      BOOST_CHECK_EQUAL( dummy , (double*)0 );
    }
  }
  ...
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, ptr_tmprcv2, r, 2);
  for (i=0; i<4*nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv2[i] , ptr_rcvdat2[i] );

  for(r=0; r<nproc; r++) {
    for (i=0; i<4*nproc; i++) ptr_tmprcv2[i]=ptr_snddat2[i];
    PE::Comm::instance().reduce(PE::plus(), ptr_tmprcv2, sndcnt, ptr_tmprcv2, r, 2);
    if (irank==r) {
      for (i=0; i<4*nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv2[i] , ptr_rcvdat2[i] );
    } else {
...
BOOST_AUTO_TEST_CASE( reduce_vector_constant )
{
  ...
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), vec_snddat, vec_tmprcv, r);
  ...
  for(r=0; r<nproc; r++) {
    ...
    PE::Comm::instance().reduce(PE::plus(), vec_tmprcv, vec_tmprcv, r);
    if (irank==r) {
      for (i=0; i<2*nproc; i++)  BOOST_CHECK_EQUAL( vec_tmprcv[i] , vec_rcvdat[i] );
    } else {
  ...
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), vec_snddat2, vec_tmprcv2, r, 2);
  ...
  for(r=0; r<nproc; r++) {
    ...
    PE::Comm::instance().reduce(PE::plus(), vec_tmprcv2, vec_tmprcv2, r, 2);
    if (irank==r) {
      for (i=0; i<4*nproc; i++)  BOOST_CHECK_EQUAL( vec_tmprcv2[i] , vec_rcvdat2[i] );
    } else {
...
BOOST_AUTO_TEST_CASE( reduce_ptr_variable )
{
  ...
  for(r=0; r<nproc; r++) {
    if (r==irank) {
      ptr_tmprcv=PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, ptr_sndmap, (double*)0, ptr_rcvmap, r);
    } else {
      double* dummy=PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, ptr_sndmap, (double*)0, ptr_rcvmap, r);
      BOOST_CHECK_EQUAL( dummy , (double*)0 );
    }
  }
  ...
  ptr_tmprcv=new double[2*nproc];
  for (i=0; i<2*nproc; i++) ptr_tmprcv[i]=0.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), ptr_snddat, sndcnt, ptr_sndmap, ptr_tmprcv, ptr_rcvmap, r);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv[2*i] , ptr_rcvdat[2*i] );

  for(r=0; r<nproc; r++) {
    for (i=0; i<2*nproc; i++) ptr_tmprcv[i]=ptr_snddat[i];
    PE::Comm::instance().reduce(PE::plus(), ptr_tmprcv, sndcnt, ptr_sndmap, ptr_tmprcv, ptr_rcvmap, r);
    if (irank==r) {
      for (i=0; i<nproc; i++)  BOOST_CHECK_EQUAL( ptr_tmprcv[2*i] , ptr_rcvdat[2*i] );
    } else {
  ...
  for(r=0; r<nproc; r++) {
    if (r==irank) {
      ptr_tmprcv2=PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, ptr_sndmap, (double*)0, ptr_rcvmap, r, 2);
    } else {
      double* dummy=PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, ptr_sndmap, (double*)0, ptr_rcvmap, r, 2);
      BOOST_CHECK_EQUAL( dummy , (double*)0 );
    }
  }
  ...
  ptr_tmprcv2=new double[4*nproc];
  for (i=0; i<4*nproc; i++) ptr_tmprcv2[i]=0.;
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), ptr_snddat2, sndcnt, ptr_sndmap, ptr_tmprcv2, ptr_rcvmap, r, 2);
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv2[4*i+0] , ptr_rcvdat2[4*i+0] );
  for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv2[4*i+1] , ptr_rcvdat2[4*i+1] );

  for(r=0; r<nproc; r++) {
    for (i=0; i<4*nproc; i++) ptr_tmprcv2[i]=ptr_snddat2[i];
    PE::Comm::instance().reduce(PE::plus(), ptr_tmprcv2, sndcnt, ptr_sndmap, ptr_tmprcv2, ptr_rcvmap, r, 2);
    if (irank==r) {
      for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv2[4*i+0] , ptr_rcvdat2[4*i+0] );
      for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( ptr_tmprcv2[4*i+1] , ptr_rcvdat2[4*i+1] );
    } else {
...
BOOST_AUTO_TEST_CASE( reduce_vector_variable )
{
  ...
  vec_tmprcv.resize(0);
  vec_tmprcv.reserve(0);
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), vec_snddat, vec_sndmap, vec_tmprcv, vec_rcvmap, r);
  ...
  for(r=0; r<nproc; r++) {
    vec_tmprcv=vec_snddat;
    PE::Comm::instance().reduce(PE::plus(), vec_tmprcv, vec_sndmap, vec_tmprcv, vec_rcvmap, r);
    if (irank==r) {
      for (i=0; i<nproc; i++)  BOOST_CHECK_EQUAL( vec_tmprcv[2*i] , vec_rcvdat[2*i] );
    } else {
  ...
  vec_tmprcv2.resize(0);
  vec_tmprcv2.reserve(0);
  for(r=0; r<nproc; r++) PE::Comm::instance().reduce(PE::plus(), vec_snddat2, vec_sndmap, vec_tmprcv2, vec_rcvmap, r, 2);
  ...
  for(r=0; r<nproc; r++) {
    vec_tmprcv2=vec_snddat2;
    PE::Comm::instance().reduce(PE::plus(), vec_tmprcv2, vec_sndmap, vec_tmprcv2, vec_rcvmap, r, 2);
    if (irank==r) {
      for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( vec_tmprcv2[4*i+0] , vec_rcvdat2[4*i+0] );
      for (i=0; i<nproc; i++) BOOST_CHECK_EQUAL( vec_tmprcv2[4*i+1] , vec_rcvdat2[4*i+1] );
    } else {
