...
BOOST_AUTO_TEST_CASE( constructor )
{
  ...
  // 1. check the range
  BOOST_CHECK_EQUAL( Uint(spin_box->minimum()), Consts::uint_min() );
  BOOST_CHECK_EQUAL( Uint(spin_box->maximum()), Consts::uint_max() );

  // 2. value is empty, the line edit should be empty as well
  BOOST_CHECK_EQUAL( Uint(spin_box->value()), 0 );
...
BOOST_AUTO_TEST_CASE( set_value )
{
  ...
  // 1. check with ints
  //
  BOOST_CHECK( value->set_value( Uint(1456) ));
  BOOST_CHECK_EQUAL( Uint(spin_box->value()), Uint(1456) );

  BOOST_CHECK( value->set_value( Uint(215468548)) );
  BOOST_CHECK_EQUAL( Uint(spin_box->value()), Uint(215468548) );
...
BOOST_AUTO_TEST_CASE( value )
{
  ...
  BOOST_CHECK( theValue.type() == QVariant::UInt );
  BOOST_CHECK_EQUAL( theValue.toUInt(), 0 );

  // get value when the line edit has a string
  spin_box->setValue(488654);
  theValue = value->value();
  BOOST_CHECK( theValue.type() == QVariant::UInt );
  BOOST_CHECK_EQUAL( theValue.toUInt(), 488654 );
