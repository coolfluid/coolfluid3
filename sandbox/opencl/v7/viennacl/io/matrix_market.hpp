...
namespace viennacl
{
  namespace io
  {
    ...
    */
    ...
    long read_matrix_market_file_impl(MatrixType & mat, const char * file)
    {
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                std::cerr << "Error in file " << file << " at line " << linenum << ": Column index out of bounds: " << row << " (matrix dim: " << mat.size1() << " x " << mat.size2() << ")" << std::endl;
                return 0;
              }
    ...
    */
    template <typename MatrixType>
    long read_matrix_market_file(MatrixType & mat, const char * file)
    {
      return read_matrix_market_file_impl(mat, file);  
    }

    template <typename MatrixType>
    long read_matrix_market_file(MatrixType & mat, const std::string & file)
    {
      return read_matrix_market_file_impl(mat, file.c_str());
    }
    
    template <typename ScalarType>
    long read_matrix_market_file(std::vector< std::map<unsigned int, ScalarType> > & mat, const char * file)
    {
      viennacl::tools::sparse_matrix_adapter<ScalarType> adapted_matrix(mat);
      return read_matrix_market_file_impl(adapted_matrix, file);
    }
    
    template <typename ScalarType>
    long read_matrix_market_file(std::vector< std::map<unsigned int, ScalarType> > & mat, const std::string & file)
    {
      viennacl::tools::sparse_matrix_adapter<ScalarType> adapted_matrix(mat);
    ...
    ////////// writer /////////////
    template <typename MatrixType>
    void write_matrix_market_file_impl(MatrixType const & mat, const char * file)
    {
      ...
      for (typename MatrixType::const_iterator1 row_it = mat.begin1();
        ...
        for (typename MatrixType::const_iterator2 col_it = row_it.begin();
              col_it != row_it.end();
              ++col_it)
          writer << col_it.index1() << " " << col_it.index2() << " " << *col_it << std::endl;
      
      writer.close();
    }

    template <typename ScalarType>
    void write_matrix_market_file(std::vector< std::map<unsigned int, ScalarType> > const & mat, const char * file)
    {
      viennacl::tools::const_sparse_matrix_adapter<ScalarType> adapted_matrix(mat);
      return write_matrix_market_file_impl(adapted_matrix, file);
    }

    template <typename ScalarType>
    void write_matrix_market_file(std::vector< std::map<unsigned int, ScalarType> > const & mat, const std::string & file)
    {
      viennacl::tools::const_sparse_matrix_adapter<ScalarType> adapted_matrix(mat);
    ...
    */
    template <typename MatrixType>
    void write_matrix_market_file(MatrixType const & mat, const std::string & file)
    {
      write_matrix_market_file_impl(mat, file.c_str());
    }
