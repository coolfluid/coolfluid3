...
struct Variables::Implementation
{
  ...
  void register_variable(const std::string& name, std::string& symbol, std::string& field_name, const VariableTypesT var_type, const bool is_state)
  {
    if(m_variable_types.insert(std::make_pair(name, var_type)).second) // Check if the variable was registered
    {
      ...
      m_component.options().add_option< OptionT<std::string> >(lower_name + std::string("_field_name"), field_name)
        ->set_pretty_name(name + std::string(" Field Name"))
        ->set_description("Field name for variable " + name)
        ->link_to(&field_name);

      m_component.options().add_option< OptionT<std::string> >(lower_name + std::string("_variable_name"), symbol)
    ...
    else
    {
      ...
      m_component.option(lower_name + std::string("_field_name")).link_to(&field_name);
      m_component.option(lower_name + std::string("_variable_name")).link_to(&symbol);
    }
  }
/*
  void create_fields()
...
void Variables::register_variable(const std::string& var_name, std::string& symbol, std::string& field_name, const Variables::VariableTypesT var_type, const bool is_state)
{
  m_implementation->register_variable(var_name, symbol, field_name, var_type, is_state);
}

bool Variables::is_state_variable(const std::string& var_name) const
{
  return m_implementation->is_state_variable(var_name);
}

Uint Variables::offset(const std::string& var_name) const
{
  return m_implementation->offset(var_name);
}

Uint Variables::nb_dof() const
{
  return m_implementation->m_nbdofs;
}

Variables::VariableTypesT Variables::variable_type(const std::string& var_name) const
{
...
void Variables::state_fields(std::vector< std::string >& fieldlist) const
{
  BOOST_FOREACH(std::string var_name, m_implementation->m_state_variables)
  {
    boost::to_lower(var_name);
    const std::string field_name = m_options[var_name + std::string("_field_name")].value_str();
