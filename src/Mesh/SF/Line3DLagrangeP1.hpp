...
/// line element that lives in 3D space
/// @author Andrea Lani
...
struct Line3DLagrangeP1  : public Line3D
{
  ...
  /// In the case of the Line3D element, this is the vector corresponding to the line segment
  ...
  /// 1D elements in 3D space or 3D elements in 3D space
  template<typename NodesT>
  ...
  /// The length of an element that represents a line in the solution space, i.e.
  /// 1D elements in 3D space
  template<typename NodesT>
  static Real length(const NodesT& nodes)
  {
    Real dx = nodes[1][XX] - nodes[0][XX];
    Real dy = nodes[1][YY] - nodes[0][YY];
    Real dz = nodes[1][ZZ] - nodes[0][ZZ];
