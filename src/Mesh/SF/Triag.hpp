...
#ifndef CF_Mesh_SF_Triag_hpp
#define CF_Mesh_SF_Triag_hpp
...
/// representation of the solution and/or the geometry in a P1 (Triagar)
/// Triag element.
/// @author Willem Deconinck
/// @author Tiago Quintino
/// @author Bart Janssens
template <Uint DIM, typename SF>
struct Triag : public Mesh::ElementType {
public:
  
  typedef boost::shared_ptr< Triag > Ptr;
  typedef boost::shared_ptr< Triag const> ConstPtr;
  
  Triag(const std::string& name = type_name()) : Mesh::ElementType(name)
  {
    m_shape = shape;
  ...
  static const Uint nb_faces = (DIM == DIM_2D ? 3 : 1);
