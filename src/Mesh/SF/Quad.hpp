...
#ifndef CF_Mesh_SF_Quad_hpp
#define CF_Mesh_SF_Quad_hpp
...
/// representation of the solution and/or the geometry in a P1 (Quadar)
/// Quad element.
/// @author Willem Deconinck
/// @author Tiago Quintino
/// @author Bart Janssens
template <Uint DIM, typename SF>
struct Quad : public Mesh::ElementType {
public:
  
  typedef boost::shared_ptr< Quad > Ptr;
  typedef boost::shared_ptr< Quad const> ConstPtr;
  
  Quad(const std::string& name = type_name()) : Mesh::ElementType(name)
  {
    m_shape = shape;
  ...
  static const Uint nb_faces = (DIM == DIM_2D ? 4 : 1);
