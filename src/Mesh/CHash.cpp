    ...
    m_nb_parts(PE::instance().size())
{
  ...
  m_properties["Number of Partitions"].as_option().link_to( &m_nb_parts );
  m_properties["Number of Objects"].as_option().link_to( &m_nb_obj );
...
{
  return (proc_of_obj(obj) == PE::instance().rank());
}
