...
void CPartitioner::set_partitioning_params()
{
  ...
  zoltan_handle().Set_Param( "DEBUG_LEVEL", to_str(properties().value<Uint>("debug_level")));
  ...
  zoltan_handle().Set_Param( "RETURN_LISTS", "ALL");
  ...
  zoltan_handle().Set_Param( "NUM_GLOBAL_PARTS", to_str(properties().value<Uint>("nb_parts")));
  // The total number of parts to be generated by a call to Zoltan_LB_Partition.


  /// Zoltan graph parameters

  zoltan_handle().Set_Param( "GRAPH_PACKAGE",properties().value<std::string>("graph_package"));
  ...
  zoltan_handle().Set_Param( "CHECK_GRAPH", to_str(std::max(2u,properties().value<Uint>("debug_level"))));
...
void CPartitioner::migrate()
{
  ...
  if (properties().value<Uint>("nb_parts") == 1)
    return;
  ...
  zoltan_handle().Set_Obj_Size_Multi_Fn ( get_elems_sizes , this );
  zoltan_handle().Set_Pack_Obj_Multi_Fn ( pack_elems_messages , this );
  zoltan_handle().Set_Unpack_Obj_Multi_Fn ( unpack_elems_messages , this );

  CFdebug.setFilterRankZero(false);

  zoltan_handle().Migrate( numImport, importGlobalIds, importLocalIds, importProcs, importToPart,
                 numExport, exportGlobalIds, exportLocalIds, exportProcs, exportToPart);
  ...
  zoltan_handle().Invert_Lists (num_request,
                                request_global_ids,
                                request_local_ids,
                                request_from_procs,
                                request_to_part,
...
//    CFdebug << CFendl;
  ...
  zoltan_handle().Set_Obj_Size_Multi_Fn ( get_nodes_sizes , this );
  zoltan_handle().Set_Pack_Obj_Multi_Fn ( pack_nodes_messages , this);
  zoltan_handle().Set_Mid_Migrate_PP_Fn ( mid_migrate_nodes , this );
  // zoltan_handle().Set_Mid_Migrate_PP_Fn ( Migrate_PP_Fn ( NULL , this );
  zoltan_handle().Set_Unpack_Obj_Multi_Fn ( unpack_nodes_messages , this );


  CFdebug.setFilterRankZero(false);

  zoltan_handle().Migrate( num_request, request_global_ids, request_local_ids, request_from_procs, request_to_part,
                 num_send,    send_global_ids,    send_local_ids,    send_to_procs,      send_to_part);

  CFdebug.setFilterRankZero(true);

  ZoltanHandle::LB_Free_Part(&request_global_ids, &request_local_ids, &request_from_procs, &request_to_part);
  ZoltanHandle::LB_Free_Part(&send_global_ids, &send_local_ids, &send_to_procs, &send_to_part);
