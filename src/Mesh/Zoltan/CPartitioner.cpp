...
void CPartitioner::set_partitioning_params()
{
  ...
  zoltan_handle().Set_Param( "DEBUG_LEVEL", to_str(property("debug_level").value<Uint>()));
  ...
  zoltan_handle().Set_Param( "NUM_GLOBAL_PARTS", to_str(property("nb_parts").value<Uint>()));
  // The total number of parts to be generated by a call to Zoltan_LB_Partition.


  /// Zoltan graph parameters

  zoltan_handle().Set_Param( "GRAPH_PACKAGE",property("graph_package").value<std::string>());
  ...
  zoltan_handle().Set_Param( "CHECK_GRAPH", to_str(std::max(2u,property("debug_level").value<Uint>())));
