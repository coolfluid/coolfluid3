...
BOOST_AUTO_TEST_CASE( integrateConst )
{
  ...
  BOOST_CHECK_LT(boost::accumulators::max(CF::Tools::Testing::test(result, vol).ulps), 1);
}
...
BOOST_AUTO_TEST_CASE( computeJacobianDeterminant )
{
  ...
  const std::vector<CArray::Row> noderows = boost::assign::list_of(coord[0])(coord[1])(coord[2]);
  const Real vol = CF::Mesh::VolumeComputer<Triag2D>::computeVolume(noderows);
  BOOST_CHECK_LT(boost::accumulators::max(CF::Tools::Testing::test(0.5*TriagP1::computeJacobianDeterminant(mapped_coords, nodes), vol).ulps), 5);
}
