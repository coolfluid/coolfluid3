...
public:
  ...
  static std::string getFullName() { return "P1-Line2D"; }
...
Real VolumeComputer<P1::Line2D>::computeVolume(const std::vector<CArray::Row>& coord)
{
  // return the distance between the 2 nodes
  return std::sqrt(std::pow(coord[1][XX]-coord[0][XX],2)+std::pow(coord[1][YY]-coord[0][YY],2));
}
