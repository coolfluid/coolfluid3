...
void CWriter::write_zone(const CRegion& region)
{
  ...
  Uint idx=0;
  BOOST_FOREACH(const CTable<Real>& coordinates, find_components_recursively_with_tag<CTable<Real> >(region,Mesh::Tags::coordinates()))
  {
  ...
  {
    grouped_elements_map[elements.parent().uri().path()].push_back(elements.as_ptr<CElements const>());
  }
