  ...
  m_implementation(new Implementation(*this))
{
  ...
  regist_signal( "load_mesh" )
      ->connect( boost::bind( &FieldManager::signal_load_mesh, this, _1 ) )
  ...
  regist_signal( "write_mesh" )
      ->connect( boost::bind( &FieldManager::signal_write_mesh, this, _1 ) )
      ->description("Load a new mesh")
      ->pretty_name("Load Mesh")
...
CMesh& FieldManager::load_mesh( const URI& file, const std::string& name )
{
...
/// @todo FieldManager writes only the first mesh. Handle multiple-mesh case.

void FieldManager::write_mesh(const URI& file)
{
...
void FieldManager::signal_load_mesh ( Common::SignalArgs& node )
{
  SignalOptions options( node );
...
void FieldManager::signal_write_mesh(SignalArgs& node)
{
  SignalOptions options( node );
  URI fileuri = options.value<URI>("file");
  write_mesh(fileuri);
