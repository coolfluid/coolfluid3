#ifndef CF_Math_LExprOp_hh
#define CF_Math_LExprOp_hh
...
namespace CF {
  ...
  namespace Math {
    ...
    template <class T, class TAG, Uint N> class MatrixLET;
    template <class T, class TAG, Uint N> class VectorLET;

////////////////////////////////////////////////////////////////////////////////

/// Definition of an empty tag class
/// @author Andrea Lani
template <class T1, Uint T2> struct Tag {};
...
///
/// @author Andrea Lani
template <class DATA, class T, Uint NUM>
class LExpr {
public:
  enum {SIZE=NUM};
};
...
public:
  ...
  static inline T at(Uint i)  {return _value;}
...
///
/// @author Andrea Lani
template <class V2, class T, class TAG, Uint N>
class MVMult :
  ...
  public:
    ...
    static inline T at(Uint i) {
      T res = T();
      const Uint n = MatrixLET<T,TAG,N>::nbCols();
...
public:
  ...
  static inline T at(Uint i) {
    ...
    T res = T();
    const Uint jc = i%M3::nbCols();
    for (Uint j = 0, k = (i/M3::nbCols())*M1::nbCols();
  j < M1::nbCols(); ++j, ++k) {
...
/// @author Andrea Lani
template <class T, class TAG1, class TAG2, Uint N1, Uint N2>
...
public:
  ...
  static inline T at(Uint i) {
    ...
    T res = T();
    const Uint jc = i%M2::nbCols();
    for (Uint j = 0, k = (i/M2::nbCols())*M1::nbCols();
  j < M1::nbCols(); ++j, ++k) {
