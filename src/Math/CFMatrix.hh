#ifndef COOLFluiD_MathTools_CFMatrix_hh
#define COOLFluiD_MathTools_CFMatrix_hh
...
#include "MathTools/CFSliceMatrix.hh"
#include "MathTools/BinaryOp.hh"
...
public:
  ...
  friend std::ostream& operator<< LTGT (std::ostream& out, const CFMatrix<T>& A);

  /// Overloading of the stream operator ">>" for the input
  friend std::istream& operator>> LTGT (std::istream& in, CFMatrix<T>& A);

  /// Overloading of the "==" operator.
  /// @return true if all elements are equal elementwise
  friend bool operator== LTGT (const CFMatrix<T>& A, const CFMatrix<T>& B);
  ...
  friend void copy LTGT (const CFMatrix<T>& orig, CFMatrix<T>& dest);
  ...
  CFMatrix(CFuint mn, CFuint nn, T value = T());

  /// Constructor by size and array initializer
  /// @param mn CFuint with rows of matrix described in array
  /// @param nn CFuint with columns of matrix described in array
  /// @param init T* array storing dynamical allocated memory
  CFMatrix(bool owner, CFuint mn, CFuint nn, T* init);
...
#undef MAT_VEC_MAT_PROD
  ...
  CFuint nbRows() const { return m_rows; }

  /// Gets the number of Columns.
  /// @return the number of columns in the matrix
  CFuint nbCols() const { return m_cols; }

  /// Function returning a slice of this CFMatrix
  CFSliceMatrix<T> slice (const CFuint iStart, const CFuint jStart)
  {
  ...
  CFVector<T> getRow(const CFuint iRow) const;
  ...
  CFVector<T> getColumn(const CFuint iCol) const;

  /// Puts in a supplied CFVector, the copy of the row of the CFMatrix.
  /// @param iRow  number of the row
  void putRow(const CFuint iRow, CFVector<T>& v) const;

  /// Puts in a supplied CFVector, the copy of the column of the CFMatrix.
  /// @param iCol  number of the column
  void putColumn(const CFuint iCol, CFVector<T>& v) const;

  /// Set a row of the matrix.
  /// @param row CFVector with the row to set
  /// @param iRow  number of the row
  void setRow(const CFVector<T>& row, const CFuint iRow);

  /// Set a column of the matrix.
  /// @param col CFVector with the column to set
  /// @param iCol  number of the column
  void setColumn(const CFVector<T>& col, const CFuint iCol);

  /// Add a row to the matrix.
  /// @param row CFVector with the row to set
  /// @param iRow  number of the row
  void addRow(const CFVector<T>& row, const CFuint iRow);

  /// Add a column to the matrix.
  /// @param col CFVector with the column to set
  /// @param iCol  number of the column
  void addColumn(const CFVector<T>& col, const CFuint iCol);

  /// Set a column of the given matrix in the current one.
  /// @param col CFVector with the column to set
  /// @param iCol  number of the column
  void setColumn(CFuint iCol1, const CFMatrix<T>& m1, CFuint iCol2)
  {
    for (size_t i = 0; i < m_rows; ++i) {
  ...
  void copyRowTo(CFVector<T>& v, const CFuint& i) const;
  /// Copies the jth column into the passed vector
  void copyColumnTo(CFVector<T>& v, const CFuint& j) const;
  /// Copies the diagonal into the passed vector
  void copyDiagonalTo(CFVector<T>& v, const CFuint begin, const CFuint end);
  ...
  void resize(const CFuint rows, const CFuint cols, bool owner = true);
  
  /// Gets the size of the CFMatrix
  /// @return CFuint with size of the std::valarray stored inside
  CFuint size() const { return m_rows*m_cols; }
...
CFMatrix<T>::CFMatrix(const Expr<EXPR,T>& expr) : 
  ...
  m_owner(true)
{ 
  const CFuint sizeN = static_cast<CFuint>(std::sqrt(static_cast<T>(expr.size())));
  m_rows = sizeN;
  m_cols = sizeN;
  m_data = CFNULL;
...
template<class T>
inline T& CFMatrix<T>::operator()(CFuint i, CFuint j)
{
  cf_assert(i < m_rows);
  cf_assert(j < m_cols);
...
template<class T>
inline T CFMatrix<T>::operator()(CFuint i, CFuint j) const
{
  cf_assert(i < m_rows);
  cf_assert(j < m_cols);
  return m_data[i*m_cols+j];
...
template<class T>
inline T& CFMatrix<T>::operator[](CFuint i)
{
  cf_assert(i < size());
  return m_data[i];
}
...
template<class T>
inline T CFMatrix<T>::operator[](CFuint i) const
{
  cf_assert(i < size());
  return m_data[i];
}
...
template<class T>
inline void CFMatrix<T>::resize(const CFuint rows, 
				const CFuint cols,
				bool owner)
{
  m_owner = owner;
...
template<class T>
inline CFVector<T> CFMatrix<T>::getRow(const CFuint iRow) const
{
...
template<class T>
inline CFVector<T> CFMatrix<T>::getColumn(const CFuint iCol) const
{
...
template<class T>
inline void CFMatrix<T>::putRow(const CFuint iRow, CFVector<T>& v) const
{
  cf_assert(iRow < m_rows);
...
template<class T>
inline void CFMatrix<T>::putColumn(const CFuint iCol, CFVector<T>& v) const
{
  cf_assert(iCol < m_cols);
...
template<class T>
inline void CFMatrix<T>::setRow(const CFVector<T>& row, const CFuint iRow)
{
...
template<class T>
inline void CFMatrix<T>::setColumn(const CFVector<T>& col, const CFuint iCol)
{
...
template<class T>
inline void CFMatrix<T>::addRow(const CFVector<T>& row, const CFuint iRow)
{
  cf_assert(row.size() == m_cols);
  size_t istart = iRow*m_cols;
  for (size_t i = 0; i < m_cols; ++i, ++istart) {
...
template<class T>
inline void CFMatrix<T>::addColumn(const CFVector<T>& col, const CFuint iCol)
{
  cf_assert(col.size() == m_rows);
  for (size_t i = 0; i < m_rows; ++i) {
    m_data[iCol+m_cols*i] += col[i];
...
template <class T>
inline
void CFMatrix<T>::copyRowTo(CFVector<T>& v, const CFuint& i) const
{
...
template <class T>
inline
void CFMatrix<T>::copyColumnTo(CFVector<T>& v, const CFuint& j) const
{
...
template <class T>
inline
void CFMatrix<T>::copyDiagonalTo(CFVector<T>& v, const CFuint begin, const CFuint end)
{
  cf_assert( begin <= end );
  cf_assert( end < std::min<CFuint> ( nbRows(), nbCols() ));
...
inline void CFMatrix<T>::factorizeLU()
{
  ...
  for (size_t iDiag = 0; iDiag < resSizeM1; ++iDiag) {
    const CFreal invDiag = 1.0/(*this)(iDiag,iDiag);
    for (size_t iRow = iDiag+1; iRow < size; ++iRow) {
      const CFreal factor = (*this)(iRow,iDiag)*invDiag;
...
#endif // COOLFluiD_MathTools_CFMatrix_hh
