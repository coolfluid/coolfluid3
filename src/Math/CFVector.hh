#ifndef COOLFluiD_MathTools_CFVector_hh
#define COOLFluiD_MathTools_CFVector_hh
...
#include "MathTools/CFSliceVector.hh"
...
#undef VEC_EQ_OP_CONST
  ...
  CFSliceVector<T> slice (const CFuint& start)
  {
  ...
  /// @param iElem index
  T operator[] (const CFuint& iElem) const
  {
    cf_assert(iElem < size());
    return m_data[iElem];
  }
  ...
  CFuint size() const { return m_size; }
  ...
  void resize (const CFuint size, const T& value = T())
  {
  ...
  friend void copy LTGT (const CFVector<T>& orig, CFVector<T>& dest);
  ...
  T partialSum(const CFuint iStart, const CFuint iEnd) const;
...
template <class T>
CFVector<T>::CFVector ( const CFuint size ) : Expr<CFVector<T>,T>(*this),
  m_owner (true),
  m_size  (size),
  m_data  (CFNULL)
{
...
template <class T>
CFVector<T>::CFVector(const CFuint size, T* ptr) :
  Expr<CFVector<T>,T>(*this),
  m_owner (false),
  m_size  (size),
  m_data  (ptr)
...
template <class T>
CFVector<T>::CFVector ( const T& value, const CFuint size ) :
  Expr<CFVector<T>,T>(*this),
  m_owner (true),
  m_size  (size),
  m_data  (CFNULL)
...
template <class T>
inline T CFVector<T>::partialSum(const CFuint iStart, const CFuint iEnd) const
{
  T result = T();
...
#endif // COOLFluiD_MathTools_CFVector_hh
