...
{
  connect(&m_listener, SIGNAL(newFrame(const MPI::Intercomm &, const CF::Common::BuilderParserFrameInfo &)),
          this, SLOT(processFrame(const MPI::Intercomm &, const CF::Common::BuilderParserFrameInfo &)));
                              ...
                              unsigned int nbProcs, const QString & hosts)
{
  ...
  if(!subSysName.isEmpty() && !m_subSysComms.contains(subSysName))
  {
    ...
    // PE::getInstance().spawn(nbProcs, hosts.toStdString().c_str());
