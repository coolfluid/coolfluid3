...
URI SelectPathDialog::show(const URI & path)
{
  ...
  m_editPath->setText(path.string_without_protocol().c_str());
...
{
  m_nodeClicked = true;
  m_editPath->setText(m_treeView->selectedPath().string_without_protocol().c_str());
}
...
void SelectPathDialog::pathChanged(const QString & path)
{
  ...
  else
  {
    ...
    try
    {
      ...
      if(newPath == "/")
        newPath = root->full_path().string_without_protocol().c_str();
