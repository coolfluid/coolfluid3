...
: QDialog(parent)
{
  ...
  m_panels[ CLOSE_SAVE_FILE ] = CFNULL;
  m_panels[ CLOSE_COMMIT ] = CFNULL;
  m_panels[ CLOSE_SHUT_DOWN ] = CFNULL;
...
{
  if(m_panels[ CLOSE_COMMIT ] == CFNULL &&
     m_panels[ CLOSE_SAVE_FILE ] == CFNULL &&
     m_panels[ CLOSE_SHUT_DOWN ] == CFNULL)
    return true;
