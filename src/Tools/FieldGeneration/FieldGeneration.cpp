...
{
  OptionURI::Ptr mesh_path = boost::dynamic_pointer_cast<OptionURI>( properties().add_option<OptionURI>("Mesh", "Mesh to add the field to", std::string()) );
  mesh_path->supported_protocol(CF::Common::URI::Scheme::CPATH);
  mesh_path->mark_basic();
    
  properties().add_option< OptionT<std::string> >("FieldName", "Name of the field to create", "")->mark_basic();
  properties().add_option< OptionT<std::string> >("VariableName", "Name of the variable to create", "")->mark_basic();
  properties().add_option< OptionT<Real> >("Value", "Value for every node in the field", 0.)->mark_basic();
...
{
  Component::Ptr mesh_component = access_component_ptr(URI(property("Mesh").value_str()));
  if(!mesh_component)
  ...
  const std::string field_name = property("FieldName").value_str();
