...
////////////////////////////////////////////////////////////////////////////////
  ...
  **/
  template<typename T>
  inline void
  gathervm_impl(const PE::Communicator& comm, const T* in_values, const int *in_n, const int *in_map, T* out_values, const int *out_n, const int *out_map, const int stride )
  {
    // get data type and number of processors
    MPI_Datatype type = mpi::get_mpi_datatype(*in_values);
    int nproc;
...
template<typename T>
inline T*
gather(const PE::Communicator& comm, const T* in_values, const int in_n, T* out_values, const int stride=1)
{
  // get nproc
  int nproc;
  MPI_CHECK_RESULT(MPI_Comm_size,(comm,&nproc));
  ...
  // call c_impl
  detail::gatherc_impl(comm, in_values, in_n, out_buf, stride);
  return out_buf;
}
...
template<typename T>
inline void
gather(const PE::Communicator& comm, const std::vector<T>& in_values, std::vector<T>& out_values, const int stride=1)
{
  ...
  // call c_impl
  detail::gatherc_impl(comm, (T*)(&in_values[0]), in_values.size()/(nproc*stride), (T*)(&out_values[0]), stride);
}
...
template<typename T>
inline T*
gather(const PE::Communicator& comm, const T* in_values, const int *in_n, const int *in_map, T* out_values, int *out_n, const int *out_map, const int stride=1);
...
template<typename T>
inline T*
gather(const PE::Communicator& comm, const T* in_values, const int *in_n, T* out_values, int *out_n, const int stride=1)
{
...
template<typename T>
inline void
gather(const PE::Communicator& comm, const std::vector<T>& in_values, const std::vector<int>& in_n, const std::vector<int>& in_map, std::vector<T>& out_values, std::vector<int>& out_n, const std::vector<int>& out_map, const int stride=1);
...
template<typename T>
inline void
gather(const PE::Communicator& comm, const std::vector<T>& in_values, const std::vector<int>& in_n, std::vector<T>& out_values, std::vector<int>& out_n, const int stride=1)
{
  // call mapped variable gather
  std::vector<int> in_map(0);
  std::vector<int> out_map(0);
  gather(comm,in_values,in_n,in_map,out_values,out_n,out_map,stride);
}
...
template<typename T>
inline T*
gather(const PE::Communicator& comm, const T* in_values, const int *in_n, const int *in_map, T* out_values, int *out_n, const int *out_map, const int stride)
{
  // number of processes
  int nproc;
  MPI_CHECK_RESULT(MPI_Comm_size,(comm,&nproc));
  ...
  if (out_sum==-nproc) {
    ...
    detail::gatherc_impl(comm,in_n,1,out_n,1);
    out_sum=0;
    for (int i=0; i<nproc; i++) out_sum+=out_n[i];
  }
  ...
  // call vm_impl
  detail::gathervm_impl(comm, in_values, in_n, in_map, out_buf, out_n, out_map, stride);
  return out_buf;
}
...
template<typename T>
inline void
gather(const PE::Communicator& comm, const std::vector<T>& in_values, const std::vector<int>& in_n, const std::vector<int>& in_map, std::vector<T>& out_values, std::vector<int>& out_n, const std::vector<int>& out_map, const int stride)
{
  ...
  if (out_sum == -nproc){
    ...
    detail::gatherc_impl(comm,&in_n[0],1,&out_n[0],1);
    out_sum=0;
    boost_foreach( int & i, out_n ) out_sum+=i;
  }
  ...
  // call vm_impl
  detail::gathervm_impl(comm, (T*)(&in_values[0]), &in_n[0], &in_map[0], (T*)(&out_values[0]), &out_n[0], &out_map[0], stride);
}
