...
public:
    ...
    inline unsigned set_cursor(const unsigned Idx) { const unsigned m = (argv.size()? argv.size()-1:0); cursor = (Idx>m? m:Idx); return cursor; }
    inline unsigned get_cursor() const { return cursor; }
    inline unsigned inc_cursor() { return ++cursor; }
