...
class Component_iterator
        : public boost::iterator_facade<Component_iterator<T>,        // iterator
                                       ...
                                       >
{
  typedef boost::iterator_facade<Component_iterator<T>, T, boost::random_access_traversal_tag, T&> base;
public:
  ...
  explicit Component_iterator(std::vector<boost::shared_ptr<T> > vec, const Uint startPosition)
...
private:
  ...
  template <typename T2>
  difference_type distance_to(Component_iterator<T2> const& other) const
  {
