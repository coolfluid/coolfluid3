...
#include "Common/ConverterTools.hh"
...
XMLNode ConverterTools::configArgsToXmlNode(const ConfigArgs & args)
{
  ...
  {
    vector<string> path;
    algorithm::split(path, itArgs->first, algorithm::is_any_of("."));
...
void ConverterTools::xmlNodeToArgs(const XMLNode & xNode, ConfigArgs& args, std::string nest)
{
  ...
  if (algorithm::starts_with(nest, "XCFcase."))
  {
    std::string value;
  ...
  for ( int nn = 0; nn < xNode.nChildNode(); ++nn)
  {
