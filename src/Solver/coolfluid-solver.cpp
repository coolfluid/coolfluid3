...
int main(int argc, char * argv[])
{
  ...
  try
  {
    ...
    volume_computer->operation().configure_property(     "Field"   ,   URI("cpath://root/mesh/volumes"   ));

    // Configure this operation (CForAllElements)
    volume_computer->configure_property(    "Regions"   , regions_to_loop );
    ...
    // Configure the operation
    scalarfield_outputer->operation().configure_property(     "Field"   ,   URI("cpath://root/mesh/volumes"   ));

    // Configure this operation (CForAllElements)
    scalarfield_outputer->configure_property(    "Regions"   , regions_to_loop );
    ...
    // Configuration
    merged_operator->configure_property(    "Regions"   , regions_to_loop );
    merged_operator->operation().operation1().configure_property(     "Field"   ,   URI("cpath://root/mesh/volumes"   ));
    merged_operator->operation().operation2().configure_property(     "Field"   ,   URI("cpath://root/mesh/volumes"   ));
