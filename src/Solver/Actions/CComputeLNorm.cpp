...
CComputeLNorm::CComputeLNorm ( const std::string& name ) : CAction(name)
{
  ...
  m_properties.add_option< OptionT<Uint> >("Order", "Order of the p-norm, zero if L-inf", 2);
...
void CComputeLNorm::execute()
{
  ...
  const Uint nbrows = m_field.lock()->data().size();
  ...
  {
  case 1: // L1
    boost_foreach(CTable<Real>::ConstRow row, m_field.lock()->data().array())
        norm += std::abs( row[0] );
    break;
  case 2: // L2
    boost_foreach(CTable<Real>::ConstRow row, m_field.lock()->data().array())
  ...
  properties().property("Norm").put_value<Real>(norm);
}
