...
public:
  ...
  void add_options(Common::OptionList& options)
  {
    ...
    for(ConstantStorage::ScalarsT::iterator it = m_constant_values.m_scalars.begin(); it != m_constant_values.m_scalars.end(); ++it)
    {
      ...
      Common::Option& option = options.check(name) ? options.option(name) : *options.add_option< Common::OptionT<Real> >(name, m_constant_values.descriptions[name], it->second);
    ...
    {
      const std::string& name = it->first;
      Common::Option& option = options.check(name) ? options.option(name) : *options.add_option< Common::OptionT<RealVector> >(name, m_constant_values.descriptions[name], it->second);
      option.link_to(&it->second);
    }
  }
