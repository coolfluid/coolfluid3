...
#ifndef cf3_sdm_lineuler_BCWallNoSlip2D_hpp
#define cf3_sdm_lineuler_BCWallNoSlip2D_hpp
...
class sdm_lineuler_API BCWallNoSlip2D : public BCStrong< BCNonUniformPhysData2D >
{
public:
  static std::string type_name() { return "BCWallNoSlip2D"; }
  BCWallNoSlip2D(const std::string& name) : BCStrong< PhysData >(name)
  {
    p.gamma = 1.4;
    options().add("gamma",p.gamma)
        .description("Specific heat reatio")
        .attach_trigger( boost::bind( &BCWallNoSlip2D::config_constants, this) );
...
private:
  ...
  {
    BCStrong<PhysData>::compute_flx_pt_phys_data(elem,flx_pt,phys_data);
...
#endif // cf3_sdm_lineuler_BCWallNoSlip2D_hpp
