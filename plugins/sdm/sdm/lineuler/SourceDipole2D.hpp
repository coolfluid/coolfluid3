...
#ifndef cf3_sdm_lineuler_SourceDipole2D_hpp
#define cf3_sdm_lineuler_SourceDipole2D_hpp
...
/// @brief Dipole source term in 2D
/// @author Willem Deconinck
///
/// @f[ S = \[ \begin{array}[c]
///     0 \\
///     eps*cos((pi/2*(x-x0)/alpha) * exp(-log(2)/alpha*(y-y0)^2) * sin(omega*t) \\
///     0 \\
///     0 
/// \end{array} \] @f]
///
class sdm_lineuler_API SourceDipole2D : public SourceTerm< PhysDataBase<4,2> >
{
public:
  static std::string type_name() { return "SourceDipole2D"; }
  SourceDipole2D(const std::string& name) : SourceTerm< PhysData >(name)
  {
    ...
    options().add("source_location",source_loc_opt)
        ...
        .attach_trigger( boost::bind( &SourceDipole2D::config_source_loc, this) )
        .mark_basic();
...
private:
  ...
  RealVectorNDIM m_source_loc;        ///< Location of Dipole
...
#endif // cf3_sdm_lineuler_SourceDipole2D_hpp
