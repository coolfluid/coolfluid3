  ...
  m_tolerance(1e-12)
{
  ...
  m_options.add_option(OptionURI::create(FlowSolver::Tags::update_coeff(),"Update Coefficient","Update coefficient to multiply with residual", URI("cpath:"), URI::Scheme::CPATH))
    ->attach_trigger ( boost::bind ( &ComputeUpdateCoefficient::config_update_coeff,   this ) );

  m_options.add_option(OptionURI::create(FlowSolver::Tags::wave_speed(),"Wave Speed","WaveSpeed needed", URI("cpath:"), URI::Scheme::CPATH))
  ...
  m_options.add_option(OptionURI::create(FlowSolver::Tags::time(),"Time","Time Tracking component", URI("cpath:"), URI::Scheme::CPATH))
    ->attach_trigger ( boost::bind ( &ComputeUpdateCoefficient::config_time,   this ) );

  m_options.add_option(OptionT<Real>::create("milestone_dt","Milestone Time Step","Limits time-steps to fall on milestones",0.));

  m_options.add_option(OptionT<bool>::create("freeze_update_coeff","Freeze Update Coefficient","Disable (re)computation of update_coefficient. Some multistage methods might want to freeze this",m_freeze))
      ->link_to(&m_freeze);
}
