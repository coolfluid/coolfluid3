...
: CWizard(name)
{
  ...
  m_model_link = create_static_component<Link>("current_model");
}
...
void SFDWizard::create_simulation()
{
  ...
  physical_model.options().configure_option("solution_state",option("solution_state").value_str());
  ...
  solver.options().configure_option(FlowSolver::Tags::physical_model(),physical_model.uri());
  solver.options().configure_option(FlowSolver::Tags::time(),time.uri());

  model.tools().create_component<InitFieldFunction>("initialize_solution");
...
void SFDWizard::prepare_simulation()
{
  ...
  std::vector<Handle< Mesh > > meshes = find_components<Mesh>(model.domain()).as_vector();
  if (meshes.size() == 0)
  ...
  if (meshes.size() > 1)
  {
    ...
    {
      model.solver().options().configure_option("mesh",option("mesh").value<URI>());
    }
    else
    {
  ...
  {
    model.solver().options().configure_option("mesh",meshes[0]->uri());
  }
...
void SFDWizard::start_simulation(const Real& end_time, const Real& time_step)
{
  ...
  model.time()
      .options().configure_option("end_time",end_time)
      .options().configure_option("time_step",time_step);

  model.solver().options().configure_option_recursively(FlowSolver::Tags::cfl(),option(FlowSolver::Tags::cfl()).value<Real>());
  model.solver().options().configure_option_recursively(FlowSolver::Tags::time_accurate(),option(FlowSolver::Tags::time_accurate()).value<bool>());
...
void SFDWizard::initialize_solution(const std::vector<std::string>& functions)
{
  ...
  init_solution.options().configure_option("functions",functions);
...
void SFDWizard::signal_start_simulation( SignalArgs& node )
{
  ...
  if (options.exists("current_time"))
    m_model_link->follow()->as_type<CModel>().options().configure_option("time",options.option<Real>("time"));
...
void SFDWizard::signature_start_simulation( SignalArgs& node )
{
  ...
  options.add_option("current_time", time.time())
     .description("Current Time" );
  options.add_option("end_time", time.option("end_time").value<Real>())
     .description("End Time" );
  options.add_option("time_step", time.option("time_step").value<Real>())
...
void SFDWizard::build_solve()
{
  ...
  RK.options().configure_option("stages",option("RK_stages").value<Uint>());
  ...
  compute_rhs.create_component <InitFieldConstant>("1.1_init_residual")
    .mark_basic()
    .options().configure_option("constant",0.)
  ...
  solver.options().configure_option_recursively("riemann_solver",std::string("cf3.RiemannSolvers.Roe"));
  solver.options().configure_option_recursively("roe_state",option("roe_state").value_str());

  solver.options().configure_option_recursively("solution_state",m_model_link->follow()->as_type<CModel>().physics().solution_state().uri());
...
void SFDWizard::build_setup()
{
  ...
  transform_mesh.create_component<BuildFaces>       ("1_build_faces").mark_basic().options().configure_option("store_cell2face",true);
  transform_mesh.create_component<CreateSpaceP0>     ("2_create_space_P0").mark_basic();
  transform_mesh.create_component<SFDM::CreateSpace> ("3_create_sfd_spaces").mark_basic().options().configure_option("P",option("P").value<Uint>());
...
void SFDSetup::execute()
{
  ...
  access_component("../iterate/1_RK_stages/1_for_each_stage/1_pre_update_actions/1_compute_rhs/1.3_for_all_cells").options().configure_option("regions",std::vector<URI>(1,mesh().topology().uri()));

  /// 3) configure the initialize_solution component. The field must be set to the solution.
  access_component("../../tools/initialize_solution").options().configure_option("field",mesh().get_child(FlowSolver::Tags::solution()).uri());
}
