  ...
  CSolver ( name )
{
  ...
  regist_signal( "initialize_solution" )
    ->connect( boost::bind( &RKRD::signal_initialize_solution, this, _1 ) )
    ->description("initializes the solution using analytical math formulas")
    ->pretty_name("Initialize the solution")
    ->signature( boost::bind( &RKRD::signature_signal_initialize_solution, this, _1));
  ...
  regist_signal( "create_boundary_term" )
    ...
    ->pretty_name("Create Boundary Condition")
      ->signature( boost::bind( &RKRD::signature_signal_create_boundary_term, this, _1));
  ...
  regist_signal( "create_domain_term" )
    ...
    ->pretty_name("Create Domain Term")
      ->signature( boost::bind( &RKRD::signature_signal_create_domain_term, this, _1));
...
void RKRD::signal_create_domain_term( SignalArgs& node )
{
  ...
  if( m_mesh.lock() )
    dterm->configure_option( RDM::Tags::mesh(), m_mesh.lock()->uri());
  if( m_physical_model.lock() )
    dterm->configure_option( RDM::Tags::physical_model() , m_physical_model.lock()->uri());
}
