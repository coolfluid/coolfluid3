...
Common::ComponentBuilder < SUPG, RDM::DomainTerm, LibRDM > SUPG_Builder;
...
struct ElementLoop
{
  ...
  SUPG& comp;
  /// Constructor
  ElementLoop( SUPG& comp_in, Mesh::CRegion& region_in ) : comp(comp_in), region(region_in) {}
  ...
  void operator() ( SF& T )
  {
    ...
    typedef SchemeSUPG< SF, QD, PHYS > SchemeT;
...
SUPG::SUPG ( const std::string& name ) : RDM::DomainTerm(name)
{
