...
wizard.create_model(model_name='Model', physical_model='cf3.physics.Scalar.Scalar3D')
model = root.get_child('Model')

### read mesh

domain = model.get_child('Domain')
mesh = domain.load_mesh(file=cf.URI('box-tet-p1-3112.msh', cf.URI.Scheme.file), name='mesh')
...
solver.options().configure_option('update_vars', 'LinearAdv3D')
...
iconds.get_child('INIT').options().configure_option('functions',['sin(x*y*z)'])
...
bcs.create_boundary_condition(name='INLET', type='cf3.RDM.BcDirichlet', regions=[
  cf.URI('//Model/Domain/mesh/topology/inlet'),
  cf.URI('//Model/Domain/mesh/topology/left')
])
bcs.get_child('INLET').options().configure_option('functions', ['cos(2*3.141592*(x+y+z))'])
