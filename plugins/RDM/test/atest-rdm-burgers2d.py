...
mesh = domain.load_mesh(file=cf.URI('square1x1-tg-p1-7614.msh', cf.URI.Scheme.file), name='mesh')
...
solver.options().set('update_vars', 'Burgers2D')
...
iconds.get_child('INIT').options().set('functions',['sin(x)'])
...
bcs.get_child('INLET').options().set('functions', ['1.5-2.0*x'])
