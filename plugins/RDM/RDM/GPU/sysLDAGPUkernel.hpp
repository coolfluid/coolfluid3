...
"                                                                                                           \n"
"               Rv[adressQS]    = 1;        Rv[adressQS+1]  = 0.0;             Rv[adressQS+2]  = ra;            Rv[adressQS+3]  = ra;                                      \n"
"               Rv[adressQS+4]  = u;        Rv[adressQS+5]  = rho*ny;          Rv[adressQS+6]  = ra * (u+a*nx); Rv[adressQS+7]  = ra * (u-a*nx);                           \n"
"               Rv[adressQS+8]  = v;        Rv[adressQS+9]  = -rho*nx;         Rv[adressQS+10] = ra * (v+a*ny); Rv[adressQS+11] = ra * (v-a*ny);                           \n"
"               Rv[adressQS+12] = 0.5*uuvv; Rv[adressQS+13] = rho*(u*ny-v*nx); Rv[adressQS+14] = ra * (H+a*um); Rv[adressQS+15] = ra * (H-a*um);                           \n"
"                                                                                                                                                                      \n"
"               Lv[adressQS]    = 1-coeffM2;                    Lv[adressQS+1]  = uDivA / a;        Lv[adressQS+2]  = vDivA / a;        Lv[adressQS+3] = (gamma-1)/(a*a);  \n"
"               Lv[adressQS+4]  = ( v*nx-u*ny ) / rho;          Lv[adressQS+5]  = ny/rho;           Lv[adressQS+6]  = -nx / rho;        Lv[adressQS+7] = 0.0;              \n"
"               Lv[adressQS+8]  = a / rho * (coeffM2 - um / a); Lv[adressQS+9]  = (nx-uDivA) / rho; Lv[adressQS+10] = (ny-vDivA) / rho; Lv[adressQS+11] = gm1_ov_rhoa;     \n"
"               Lv[adressQS+12] = a / rho * (coeffM2 + um / a); Lv[adressQS+13] = (nx+uDivA) / rho; Lv[adressQS+14] = (ny+vDivA) / rho; Lv[adressQS+15] = gm1_ov_rhoa;     \n"
"                                                                                                                            \n"
"               Dv[j*nEq*shape] = max(um,0); Dv[j*nEq*shape+1] = max(um,0); Dv[j*nEq*shape+2] = max(um+a,0); Dv[j*nEq*shape+3] =max(um-a,0);         \n"
"                                                                                                                                                    \n"
"                                                                                                                                                    \n"
"                                                                                                                                                    \n"
...
"                  for( int m = 0; m < nEq; m++ )                                                                                                    \n"
"                     for( int n = 0; n < nEq; n++ )                                                                                                 \n"
"                           Ki[adressQS+l*nEq+m] += Rv[adressQS+l*nEq+n ] * Dv[j*nEq*shape+n] * Lv[adressQ+Sm+n*nEq ];                               \n"
"               for( int l = 0; l < nEq*nEq; l++ )                                                                                                   \n"
"                   sumLplus[adressQ+l] += Ki[adressQS+l];                                                                                           \n"
"                                                                                                                                                    \n"
"               float wave_speed = max(Dv[j*nEq*shape], max(Dv[j*nEq*shape+1], max(Dv[j*nEq*shape+2],Dv[j*nEq*shape+3] ) ) ) * weights[j] *jq[j];    \n"
