...
#include "cf3/physics/NavierStokes/NavierStokes2D.hpp"
...
public: // functions
                                            ...
                                            EV& Dv)
  {
    ...
    Rv <<
          1,             0,                        ra,                  ra,
          p.u,           p.rho*ny,                 ra*(p.u + p.a*nx),   ra*(p.u - p.a*nx),
          p.v,          -p.rho*nx,                 ra*(p.v + p.a*ny),   ra*(p.v - p.a*ny),
          0.5 * p.uuvv,  p.rho*(p.u*ny - p.v*nx),  ra*(p.H + p.a*um),   ra*(p.H - p.a*um);
