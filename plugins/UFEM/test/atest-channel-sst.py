...
# Add the k-omega turbulence model solver(komega)
komega = solver.add_unsteady_solver('cf3.UFEM.SSTKOmega')
...
komega.regions = [mesh.topology.uri()]

for lss in [nstokes.LSS, komega.LSS.LSS]:
...
ic_k = solver.InitialConditions.create_initial_condition(builder_name = 'cf3.UFEM.InitialConditionFunction', field_tag = 'komega_solution')
ic_k.variable_name = 'k'
ic_k.value = [str(k_init)]
ic_k.regions = [mesh.topology.uri()]

ic_omega = solver.InitialConditions.create_initial_condition(builder_name = 'cf3.UFEM.InitialConditionFunction', field_tag = 'komega_solution')
ic_omega.variable_name = 'omega'
ic_omega.value = [str(omega_init)]
ic_omega.regions = [mesh.topology.uri()]
...
# Repartition to remove the global boundary
partitioner = domain.create_component('Partitioner', 'cf3.zoltan.PHG')
partitioner.mesh = mesh
partitioner.execute()

# Compute node valence
mesh.geometry.create_field(name = 'node_valence', variables='Valence[scalar]').add_tag('node_valence')
valence = domain.create_component('Valences', 'cf3.solver.actions.NodeValence')
valence.regions = [mesh.topology.uri()]
valence.execute()

writer = domain.create_component('VtmWriter', 'cf3.vtk.MultiblockWriter')
writer.mesh =  mesh
writer.file = cf.URI('atest-channel-sst-out.vtm')
writer.execute()

# Recompute wall->interior connectivity
...
bc.create_bc_action(region_name = 'top', builder_name = 'cf3.UFEM.BCWallFunctionSST')
...
bc = komega.LSS.BoundaryConditions
...
bc = komega.LSS.BoundaryConditions
bc.add_constant_bc(region_name = 'bottom', variable_name = 'omega').value =  omega_wall
bc.add_constant_bc(region_name = 'top', variable_name = 'omega').value =  omega_wall
...
probe0.History.file = cf.URI('atest-channel-sst.tsv')
