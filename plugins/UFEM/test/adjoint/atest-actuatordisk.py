...
ad_solver = solver1.add_unsteady_solver('cf3.UFEM.adjoint.Adjoint')
mesh = domain.load_mesh(file = cf.URI('actuator2df.msh'), name = 'Mesh')
mesh1 = domain1.load_mesh(file = cf.URI('actuator2df.msh'), name = 'Mesh')
# actve region
disk.regions = [mesh.topology.actuator.uri(), mesh.topology.actuator.uri()]
ns_solver.regions = [mesh.topology.uri()]
ad_solver.regions = [mesh1.topology.uri(), mesh1.topology.actuator.uri()]
# initial conditions
...
solver1.InitialConditions.adjoint_solution.AdjVelocity = initial_velocity
# set physical constants
physics.density = rho
physics.dynamic_viscosity = mu
physics1.density = rho
physics1.dynamic_viscosity = mu
...
time1 = model1.create_time()
time1.time_step = tstep
time1.end_time = num_steps*tstep
...
# writer.execute()
domain1.write_mesh(cf.URI('outputactutest.pvtu'))
