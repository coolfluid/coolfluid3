...
rho_gas = 2.
mu = 1.8e-5
D = 0.095e-4 # Diffusion coefficient of phosgene

c_in = 0.1 # initial concentration
...
conc = solver.add_unsteady_solver('cf3.UFEM.BoussinesqConcentration')
conc.options.theta = 1.
conc.density = rho_gas
...
conc.regions = [mesh.topology.uri()]

# solver setup
for lss in [nstokes.LSS, ke.LSS.LSS, conc.LSS]:
...
solver.InitialConditions.volume_fraction_boussinesq.volume_fraction_boussinesq = c_in
solver.InitialConditions.volume_fraction_boussinesq.regions = [mesh.topology.box.uri()]
...
bc = conc.get_child('BoundaryConditions')
bc.add_constant_bc(region_name = 'in', variable_name = 'volume_fraction_boussinesq').value =  0.
...
writer.mesh = mesh
writer.file = cf.URI('atest-ufem-abl-boussinesqconcentration-{iteration}.vtm')
...
probe0.History.file = cf.URI('abl-probe-conc.tsv')
