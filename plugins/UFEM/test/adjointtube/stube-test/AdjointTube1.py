...
# voeg de oppervlakte integraal toe
# solver.add_unsteady_solver(cf3.UFEM.surface_integral)
...
gradient2.options.gradient_name = 'v'
...
# surface.regions = [mesh.topology.bottom_curve.uri()]
...
# Adjoint oplossing
time.end_time += 1300.0 * time.time_step/10 # add again the same number of steps as in the forward solution
...
domain.write_mesh(cf.URI('parab-output-adjoint.pvtu'))
model.print_timing_tree()

# sensitivity derivatives berekenen



sensitivity_integral = solver.add_unsteady_solver('cf3.UFEM.SurfaceIntegral')
...
print sensitivity_integral.result
