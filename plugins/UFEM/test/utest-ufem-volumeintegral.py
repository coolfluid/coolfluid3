...
pressure_integral = solver.add_unsteady_solver('cf3.UFEM.VolumeIntegral')
pressure_integral.variable_name = 'Pressure'
pressure_integral.field_tag = 'navier_stokes_solution'
pressure_integral.regions = [mesh.topology.uri()]
pressure_integral.execute()

if abs(pressure_integral.result - 10.0) > 1e-10:
  raise Exception('Wrong integration result: ' + str(pressure_integral.result) + ', expected 10')

print pressure_integral.result
