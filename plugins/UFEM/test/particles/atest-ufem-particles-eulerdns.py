...
class TaylorGreen:
  ...
  def setup_model(self, builder):
    ...
    self.eulerdns = self.solver.add_unsteady_solver('cf3.UFEM.particles.EulerDNS')
  ...
  def setup(self, Ua, Va, D, theta):
    ...
    self.eulerdns.regions = [mesh.topology.interior.uri()]
...
tg_impl = TaylorGreen(builder = 'cf3.UFEM.NavierStokes', dt = dt, element=elem, prefix='EulerDNS')
tg_impl.create_mesh(segs)
tg_impl.setup(0., 0., D=0.5, theta=theta)
tg_impl.setup_ic()
tg_impl.eulerdns.options.tau_p = 0.1

for i in range(len(tg_impl.mesh.geometry.coordinates)):
