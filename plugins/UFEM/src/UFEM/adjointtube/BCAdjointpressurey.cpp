...
#include "BCAdjointpressurey.hpp"
...
common::ComponentBuilder < BCAdjointpressurey, common::Action, LibUFEMAdjointTube > BCAdjointpressurey_Builder;

BCAdjointpressurey::BCAdjointpressurey(const std::string& name) :
  ProtoAction(name),
  m_dirichlet(options().add("lss", Handle<math::LSS::System>()).pretty_name("LSS").description("The linear system for which the boundary condition is applied"))
{
    ...
    set_expression(nodes_expression(m_dirichlet(q)  = (transpose(u)*U)[0] + (u[1]*U[1]) + (nu_eff*grad_Uy[1])-u[1])); //-0.5*(transpose(u)*u)[1]-u[1]*u[1]));
...
BCAdjointpressurey::~BCAdjointpressurey()
{
}
