...
struct ComputeTau
{
  ...
  void operator()(const UT& u, const NUT& nu_eff, const Real& u_ref, const Real& dt, Real& tau_ps, Real& tau_su, Real& tau_bulk) const
  {
    ...
    const Real old_tau_ps = he*xi/(2.*u_ref);

    //std::cout << "old pspg: " << old_tau_ps << std::endl;
  }
...
//    {
//      g2 += (u.nabla().row(i).transpose()*u.shape_function()).squaredNorm();
//    }
//    const Real gamma2 = fabs((u.nabla().transpose()*(u.eval()*u.nabla())).squaredNorm());
//    const Real tau_p1_inv = gamma > 1e-13 ? sqrt(gamma2/g2) : 0.;


//    tau_ps = 1./sqrt(tau_diff);
  }
};
}
/// Placeholder for the compute_tau operation. Use as compute_tau(velocity_field, nu_eff_field, u_ref, dt, tau_ps, tau_su, tau_bulk)
static solver::actions::Proto::MakeSFOp<supgtest::ComputeTau>::type const compute_tau = {};
